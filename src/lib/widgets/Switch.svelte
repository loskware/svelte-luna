<script lang="ts">
	...
	type SwitchTheme = 'accent' | 'danger' | 'warning' | 'success';
	type SwitchSize = 'compact' | 'normal' | 'large';
...
<div bind:this={ref} class={cn} {style} class:disabled on:click={handleClick}>
	<input bind:this={input} type="checkbox" checked={state} {disabled} {...$$restProps} />
	<span class="track" />
	{#if label}
...
<style>
	...
	.track::before {
		...
		transition: background-color var(--luna-duration-20) var(--luna-timing-ease-in-out);
