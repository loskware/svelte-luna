<script lang="ts">
	...
	let anchorOption: 'bottom-left' | 'bottom-right' | 'top-left' | 'top-right' = 'bottom-left';
</script>
...
<section>
	<Card outlined backgroundColor="transparent" style="margin: 32px">
		...
		<div class="container">
			<div class="anchor-option-grid">
				<h2>Anchor</h2>
				<div>
					<Radio label="Bottom Left" value="bottom-left" bind:group={anchorOption} />
					<br />
					<Radio label="Bottom Right" value="bottom-right" bind:group={anchorOption} />
					<br />
					<Radio label="Top Left" value="top-left" bind:group={anchorOption} />
					<br />
					<Radio label="Top Right" value="top-right" bind:group={anchorOption} />
				</div>
			</div>
			<div class="menu-wrapper">
				<div>
					<Menu anchor={anchorOption} onAction={(action, _) => (selectedOption = action)}>
	...
	<Card outlined backgroundColor="transparent" style="margin: 32px">
		<div class="container">
			...
			<p class="description">
				Menus can be linked to any Svelte component or standard HTML element. Just assign them the <i
					><b>target slot</b></i
				>
			</p>
			<p class="extra">
		...
		<div class="container">
			...
			<p class="description">
				To add actions to the menu insert a <b><i>MenuItem</i></b> for each action required.
				<br />
			...
			<p class="extra">
				MenuItem can contain anything you want. Simple text, SVGs, images, as well as other
				components. MenuItem is a simple flex container with some styles.
			</p>
			<p class="extra">
				If you want to add a divider between MenuItems use the <b><i>Divider</i></b> component (can
